...
class DisplayUtils:
    ...
    def display_seating_capacity(self, current_passengers, max_capacity):
        ...
        if int(current_passengers) > int(max_capacity):
            ...
            info_font = ImageFont.truetype(self._text_font, 16)
            # Texto a mostrar
            ...
            lines = self._split_text_into_lines(draw, text_seating_capacity, info_font)
        ...
        top_info_font = ImageFont.truetype(self._text_font, 25)
        ...
        info_font = ImageFont.truetype(self._text_font, 45)
    ...
    def display_geolocation(self, current_speed, speed_limit, location_info):
        self.draw_speed_limit(speed_limit, current_speed, location_info)
    ...
    def display_road_bad_state(self):
        ...
        info_font = ImageFont.truetype(self._text_font, 20)
        # Texto a mostrar
        text_bad_road_state = "Carretera en mal estado"
        # Dividir el texto en líneas para que quepa en la pantalla
        lines = self._split_text_into_lines(draw, text_bad_road_state, info_font)
        self._draw_text_lines(draw, info_font, lines, text_x, text_y)
        self._display_image(image)
    ...
    def draw_speed_limit(self, speed_limit, current_speed, location_info: str):
        ...
        info_font = ImageFont.truetype(self._text_font, 13)
        ...
        # Dividir el texto en líneas para que quepa en la pantalla
        road_font = ImageFont.truetype(self._text_font, 16)
        lines = self._split_text_into_lines(draw, next_top_text, road_font)
        self._draw_text_lines(draw, road_font, lines, (DisplayOLEDConfig.WIDTH - top_text_width) // 2, top_text_y + top_text_height + 5)

        # Dibujar el texto del límite de velocidad
        max_speed_font = ImageFont.truetype(self._text_font, 22)
        ...
        current_speed_font = ImageFont.truetype(self._text_font, 28)
    ...
    def _speed_limit_error(self, draw: ImageDraw, image: Image, current_speed: int) -> Image:
        """
        ...
        info_font = ImageFont.truetype(self._text_font, 32)
        current_speed_font = ImageFont.truetype(self._text_font, 42)
